...
def send_email_notification(contact_data: dict):
    try:
        ...
        try:
            # Environment variables'dan email ayarlarını al
            ...
            print(f"📧 Email gönderiliyor: {smtp_username} -> {support_email}")
...
Yanıtlamak için: {contact_data['email']}
            ...
            with smtplib.SMTP(smtp_server, smtp_port) as server:
                server.starttls()
                print("🔑 Gmail'e giriş yapılıyor...")
                server.login(smtp_username, smtp_password)
                print("📤 Email gönderiliyor...")
                server.send_message(msg)
                
            print(f"✅ Email başarıyla gönderildi: {support_email}")
            return True
            
        except Exception as email_error:
            print(f"❌ Email gönderim hatası: {email_error}")
            print("📝 Mesaj sadece konsola kaydedildi")
            return True  # Konsola yazdırma başarılı olduğu için True döndür
